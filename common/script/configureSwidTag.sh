...
if [ -n "$KubeVersion" ]; then
		...
		removeAllSwidTag ibm.com_Operational_Decision_Manager_Containers_*.swidtag
		...
		if [ -n "$DEPLOY_FOR_PRODUCTION" ]; then
        		if [[ "$DEPLOY_FOR_PRODUCTION" == "TRUE" ]]; then
                		...
                		removeAllSwidTag swidtagToRemove=ibm.com_Operational_Decision_Manager_Containers_*.swidtag
        		fi
		else
