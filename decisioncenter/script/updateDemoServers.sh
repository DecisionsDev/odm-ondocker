...
retry=0
while [[ ("$(curl -X POST "$CURL_URL/decisioncenter-api/v1/servers/06465bbf-a2e6-4c3b-9e4d-df29f4f76cf7" -H "Content-Type: application/json;charset=UTF-8" -d "{ \"url\": \"$DSE_URL\",\"authenticationKind\": \"OAUTH\",\"authenticationProvider\": \"$OPENID_PROVIDER\" }" -H "authorization: Basic b2RtQWRtaW46b2RtQWRtaW4=" -w "%{http_code}" -s -o /dev/null)" != 200) && ( $retry -lt 10) ]];do 
...
retry=0
while [[ ("$(curl -X POST "$CURL_URL/decisioncenter-api/v1/servers/03ed315d-fbad-419a-84a3-52a7cedc24e7" -H "Content-Type: application/json;charset=UTF-8" -d "{ \"url\": \"$DR_URL\",\"authenticationKind\": \"OAUTH\",\"authenticationProvider\": \"$OPENID_PROVIDER\" }" -H "authorization: Basic b2RtQWRtaW46b2RtQWRtaW4=" -w "%{http_code}" -s -o /dev/null)" != 200)  && ( $retry -lt 10) ]];do
...
retry=0
while [[ ("$(curl -X POST "$CURL_URL/decisioncenter-api/v1/servers/06465bbf-a2e6-4c3b-9e4d-df29f4f76cf7" -H "Content-Type: application/json;charset=UTF-8" -d "{ \"url\": \"$DSE_URL\" }" -H "authorization: Basic b2RtQWRtaW46b2RtQWRtaW4=" -w "%{http_code}" -s -o /dev/null)" != 200) && ( $retry -lt 10) ]];do
...
retry=0
while [[ ("$(curl -X POST "$CURL_URL/decisioncenter-api/v1/servers/03ed315d-fbad-419a-84a3-52a7cedc24e7" -H "Content-Type: application/json;charset=UTF-8" -d "{ \"url\": \"$DR_URL\" }" -H "authorization: Basic b2RtQWRtaW46b2RtQWRtaW4=" -w "%{http_code}" -s -o /dev/null)" != 200)  && ( $retry -lt 10) ]];do
