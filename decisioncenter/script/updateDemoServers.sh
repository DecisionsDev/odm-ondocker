...
retry=0
while [[ ("$(curl -X POST "$CURL_URL/decisioncenter-api/v1/servers/2b407606-381e-4ff4-a082-f3cd96079576" -H "Content-Type: application/json;charset=UTF-8" -d "{ \"url\": \"$DSE_URL\",\"authenticationKind\": \"OAUTH\",\"authenticationProvider\": \"$OPENID_PROVIDER\" }" -H "authorization: Basic b2RtQWRtaW46b2RtQWRtaW4=" -w "%{http_code}" -s -o /dev/null)" != 200) && ( $retry -lt 10) ]];do 
...
retry=0
while [[ ("$(curl -X POST "$CURL_URL/decisioncenter-api/v1/servers/f0d48ec7-109b-45bb-badf-3f5cae0a1f80" -H "Content-Type: application/json;charset=UTF-8" -d "{ \"url\": \"$DR_URL\",\"authenticationKind\": \"OAUTH\",\"authenticationProvider\": \"$OPENID_PROVIDER\" }" -H "authorization: Basic b2RtQWRtaW46b2RtQWRtaW4=" -w "%{http_code}" -s -o /dev/null)" != 200)  && ( $retry -lt 10) ]];do
...
retry=0
while [[ ("$(curl -X POST "$CURL_URL/decisioncenter-api/v1/servers/2b407606-381e-4ff4-a082-f3cd96079576" -H "Content-Type: application/json;charset=UTF-8" -d "{ \"url\": \"$DSE_URL\" }" -H "authorization: Basic b2RtQWRtaW46b2RtQWRtaW4=" -w "%{http_code}" -s -o /dev/null)" != 200) && ( $retry -lt 10) ]];do
...
retry=0
while [[ ("$(curl -X POST "$CURL_URL/decisioncenter-api/v1/servers/f0d48ec7-109b-45bb-badf-3f5cae0a1f80" -H "Content-Type: application/json;charset=UTF-8" -d "{ \"url\": \"$DR_URL\" }" -H "authorization: Basic b2RtQWRtaW46b2RtQWRtaW4=" -w "%{http_code}" -s -o /dev/null)" != 200)  && ( $retry -lt 10) ]];do
