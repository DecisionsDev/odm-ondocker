...
retry=0
while [[ ("$(curl -X POST "$CURL_URL/decisioncenter-api/v1/servers/0f263d8b-1dda-493c-9e55-8b7bdd9ff721" -H "Content-Type: application/json;charset=UTF-8" -d "{ \"url\": \"$DSE_URL\",\"authenticationKind\": \"OAUTH\",\"authenticationProvider\": \"$OPENID_PROVIDER\" }" -H "authorization: Basic b2RtQWRtaW46b2RtQWRtaW4=" -w "%{http_code}" -s -o /dev/null)" != 200) && ( $retry -lt 10) ]];do 
...
retry=0
while [[ ("$(curl -X POST "$CURL_URL/decisioncenter-api/v1/servers/1bd2444a-f2d0-40fe-9cf6-9c51a1f58618" -H "Content-Type: application/json;charset=UTF-8" -d "{ \"url\": \"$DR_URL\",\"authenticationKind\": \"OAUTH\",\"authenticationProvider\": \"$OPENID_PROVIDER\" }" -H "authorization: Basic b2RtQWRtaW46b2RtQWRtaW4=" -w "%{http_code}" -s -o /dev/null)" != 200)  && ( $retry -lt 10) ]];do
...
retry=0
while [[ ("$(curl -X POST "$CURL_URL/decisioncenter-api/v1/servers/0f263d8b-1dda-493c-9e55-8b7bdd9ff721" -H "Content-Type: application/json;charset=UTF-8" -d "{ \"url\": \"$DSE_URL\" }" -H "authorization: Basic b2RtQWRtaW46b2RtQWRtaW4=" -w "%{http_code}" -s -o /dev/null)" != 200) && ( $retry -lt 10) ]];do
...
retry=0
while [[ ("$(curl -X POST "$CURL_URL/decisioncenter-api/v1/servers/1bd2444a-f2d0-40fe-9cf6-9c51a1f58618" -H "Content-Type: application/json;charset=UTF-8" -d "{ \"url\": \"$DR_URL\" }" -H "authorization: Basic b2RtQWRtaW46b2RtQWRtaW4=" -w "%{http_code}" -s -o /dev/null)" != 200)  && ( $retry -lt 10) ]];do
