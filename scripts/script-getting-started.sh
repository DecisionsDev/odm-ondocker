...
curl_result=$(curlRequest POST ${DC_URL}/decisioncenter-api/v1/decisionservices/import Loan_Validation_Service.zip)
# echo $curl_result
...
if [[ "${decisionServiceId}" == "null" ]]; then
  get_decisionserviceid_result=$(curlRequest GET ${DC_URL}/decisioncenter-api/v1/decisionservices?q=name%3ALoan%20Validation%20Service)
...
get_testSuiteId_result=$(curlRequest GET ${DC_URL}/decisioncenter-api/v1/decisionservices/${decisionServiceId}/testsuites?q=name%3AMain%20Scoring%20test%20suite)
